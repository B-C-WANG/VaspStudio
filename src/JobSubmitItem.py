...
class JobSubmit_item(VSP_Item):
    ...
    def connect_remote_project_dir_check(self):
        ...
        try:
            ...
            with open(check_file_path, "w") as f:
                f.write("1")
            self.sftp_tool.upload_all_file_to_one_remote_dir(files=[check_file_path],
                                                             remote_dir=self.remote_project_dir)
            return Checker(status=Status.PASS)
        except:
            traceback.print_exc()
            return Checker(status=Status.FAILED, window_status=Status.ERROR,
                           window_string="Can not connect to server or invalid remote project dir.")
    ...
    def down_load_and_read_nodel_dir_file(self):
        #
        try:
            self.sftp_tool.sftp_download(local_dir=self.local_project_dir, remote_dir=self.remote_project_dir,
                                         certain_file="IO")
